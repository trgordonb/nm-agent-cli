...
## [2026-09-24] graph | Regenerated graph visuals (49 nodes / 411 drawn edges); fixed wiki_graph_visualize.py
   Patched skills/llm-wiki/scripts/wiki_graph_visualize.py: --node-limit now defaults to 0 (full graph; old default 40 silently dropped 9 low-degree concept nodes); replaced undefined item_meta with a working click-detail panel (directional edge list + confidence); removed non-existent vis.DataSet.only() call that broke the live search filter; groups option now uses {color: ...} object form. Visuals omit 10 summarizes_raw edges whose targets are raw file paths outside the node set (still present in sqlite/jsonl). Verified: node --check on inline JS; all edge endpoints resolve.
