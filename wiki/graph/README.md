...
```bash
uv run --script scripts/wiki_graph_extract.py wiki/ --out wiki/graph
```
...
2. Run `uv run --script scripts/wiki_graph_lint.py wiki/` — catches unknown predicates, broken object references, missing evidence, alias collisions.
3. Run `uv run --script scripts/wiki_graph_extract.py wiki/ --out wiki/graph` — regenerates the artifacts above.
