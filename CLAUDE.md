...
- Wiki recall: when `./llm-wiki` (OKF layout) exists with pages, each user turn pre-flight matches the request against wiki pages (`WikiStore.build_context`) and injects a `<wiki_context>` block into that turn's system prompt (never archived); the agent also gets a `wiki_search` tool for deeper retrieval. `WIKI_DIR` overrides the location; absent/empty wiki disables the layer.
